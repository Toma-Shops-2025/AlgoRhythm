-- AlgoRhythm Library (saves) — type-safe for production
-- Fixes: ERROR 42883 text = uuid
-- Run in Supabase SQL Editor for tmpdjywsnwzivetqludd

-- If a half-created saves table exists from the failed run, remove it
DROP TABLE IF EXISTS public.saves CASCADE;

-- Use TEXT ids so this works whether posts.id / auth.users.id are uuid or text
CREATE TABLE public.saves (
  user_id text NOT NULL,
  post_id text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (user_id, post_id)
);

CREATE INDEX idx_saves_user_created ON public.saves (user_id, created_at DESC);
CREATE INDEX idx_saves_post ON public.saves (post_id);

GRANT SELECT, INSERT, DELETE ON public.saves TO authenticated;
GRANT ALL ON public.saves TO service_role;

ALTER TABLE public.saves ENABLE ROW LEVEL SECURITY;

CREATE POLICY "users read own saves" ON public.saves
  FOR SELECT TO authenticated
  USING (user_id = auth.uid()::text);

CREATE POLICY "users save as themselves" ON public.saves
  FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid()::text);

CREATE POLICY "users unsave own" ON public.saves
  FOR DELETE TO authenticated
  USING (user_id = auth.uid()::text);

ALTER TABLE public.posts
  ADD COLUMN IF NOT EXISTS save_count integer NOT NULL DEFAULT 0;

CREATE OR REPLACE FUNCTION public.saves_count_trg()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF tg_op = 'INSERT' THEN
    UPDATE public.posts
    SET save_count = save_count + 1
    WHERE id::text = NEW.post_id::text;
  ELSIF tg_op = 'DELETE' THEN
    UPDATE public.posts
    SET save_count = greatest(0, save_count - 1)
    WHERE id::text = OLD.post_id::text;
  END IF;
  RETURN NULL;
END;
$$;

DROP TRIGGER IF EXISTS saves_count_ins ON public.saves;
CREATE TRIGGER saves_count_ins
  AFTER INSERT ON public.saves
  FOR EACH ROW EXECUTE FUNCTION public.saves_count_trg();

DROP TRIGGER IF EXISTS saves_count_del ON public.saves;
CREATE TRIGGER saves_count_del
  AFTER DELETE ON public.saves
  FOR EACH ROW EXECUTE FUNCTION public.saves_count_trg();
