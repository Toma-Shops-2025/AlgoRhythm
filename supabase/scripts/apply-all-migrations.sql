-- Combined AlgoRhythm migrations for NEW free Supabase project
-- Run once in SQL Editor after creating the project
-- Media stays on Cloudflare R2; this is auth + metadata only

-- ========== 20260525210805_00d00788-b9cf-48c7-acba-46ccc4ee69c3.sql ==========
...
-- Tips (placeholder â€” wired to payments later)
...
-- ========== 20260525210831_bf49d858-64e2-45c5-b30c-a1ddfb72c74f.sql ==========
...
-- ========== 20260526230316_f3d38300-d704-428b-99d0-ae062256620c.sql ==========
...
-- ========== 20260526233520_72b1afa9-07a9-4497-b627-44483cddc39e.sql ==========
...
-- ========== 20260527055554_2dac64ac-d959-4e49-b711-0ec714b4f3a8.sql ==========
...
-- ========== 20260527074311_9e5e42de-9096-4d04-bac2-6c1af560b1cc.sql ==========
...
-- ========== 20260527080159_92c20a16-a0ec-43bd-81f4-bfd92fe0ad10.sql ==========
...
-- ========== 20260527221323_77f2eedb-ec3a-471c-88da-669783cc1d40.sql ==========
...
-- ========== 20260527221345_0584d0f3-8454-460b-863c-8bb0709e823d.sql ==========
REVOKE EXECUTE ON FUNCTION public.saves_count_trg() FROM PUBLIC, anon, authenticated;

-- ========== 20260528192001_email_infra.sql ==========
...
-- ========== 20260528192016_email_infra.sql ==========
-- Email infrastructure
-- Creates the queue system, send log, send state, suppression, and unsubscribe
-- tables used by both auth and transactional emails.
...
-- ========== 20260528192251_email_infra.sql ==========
-- Email infrastructure
-- Creates the queue system, send log, send state, suppression, and unsubscribe
-- tables used by both auth and transactional emails.
...
-- ========== 20260528192310_email_infra.sql ==========
-- Email infrastructure
-- Creates the queue system, send log, send state, suppression, and unsubscribe
-- tables used by both auth and transactional emails.
...
-- ========== 20260529011247_email_infra.sql ==========
-- Email infrastructure
-- Creates the queue system, send log, send state, suppression, and unsubscribe
-- tables used by both auth and transactional emails.
...
-- ========== 20260603193209_7d7645ca-87e0-40b5-975f-196a745b1eb7.sql ==========
...
-- ========== 20260823000000_posts_id_default.sql ==========
...
-- ========== 20260823001000_posts_is_published_default.sql ==========
...
-- ========== 20260823120000_storage_media_size_limit.sql ==========
-- Raise media bucket limit for AI-rendered music videos (default is 50MB).
-- 500MB matches ViralSnap Pro upload cap and typical 3â€“5 min webm exports.
--
-- IMPORTANT: Supabase also enforces a GLOBAL file size limit in
-- Dashboard â†’ Storage â†’ Settings. Free plans cannot exceed 50MB globally.
...
-- ========== ensure saves (text-safe, IF NOT EXISTS friendly) ==========
